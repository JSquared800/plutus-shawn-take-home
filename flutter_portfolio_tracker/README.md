...
- **Advanced market filtering and sorting** — The `AssetTicker` entity already carries `openInterest`, `volume24h`, `markPx`, and `change24hPct`, and the `MarketViewModel` has a `setSort()` method — but there is no UI surface exposing it yet. With more time this would include tappable column headers, a filter bottom sheet with sliders for min volume/OI and a 24h change range, and a gainers/losers quick-toggle chip to make asset discovery significantly more powerful.
